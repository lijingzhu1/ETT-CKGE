...
# global setting
parser.add_argument("-random_seed", dest="random_seed", default=4444,type = int, help="Set random seeds")
...
parser.add_argument("-batch_size", dest="batch_size", default=1024,type =int, help="Set the batch size")
parser.add_argument("-learning_rate", dest="learning_rate", default=1e-4, type = float,help="Set the learning rate")
...
parser.add_argument("-neg_ratio", dest="neg_ratio", default=10, type=int,help="the ratio of negtive/postive facts")
...
parser.add_argument("-multi_layer_weight", dest="multi_layer_weight", default=1, type=float,help="The weight of multi layer weight")
parser.add_argument("-use_two_stage", dest="use_two_stage", default=True, help="Use two stage distill or not") # two-stage
parser.add_argument("-two_stage_epoch_num", dest="two_stage_epoch_num", default=20, type = int,help="Num of two stage epoch")
...
#MGDD
parser.add_argument('-lifelong_name', dest='lifelong_name', default='Zhu1', help='Competitor name or LKGE') #added
# parser.add_argument("-using_MAE_loss", default=False,type = bool, help="Use MAE loss or not")


#Tokened
parser.add_argument("-using_token_distillation_loss", default=False,type = bool, help="Use token distilation loss or not")
parser.add_argument("-without_div_loss", default=False,type = bool, help="Use diversity token loss or not")
parser.add_argument("-token_num", dest="token_num", default=5,type = int, help="tokenize number")
# parser.add_argument("-token_distillation_weight", dest="token_distillation_weight", default=100000, type= float,help="The weight of token distillation")
parser.add_argument("-token_distillation_weight", default=[500, 500, 1000, 1000], nargs='+',type=int,help="distillation loss weight")
parser.add_argument("-div_loss_weight", dest="div_loss_weight", default=1, type= int,help="The weight of token training diversity loss")
