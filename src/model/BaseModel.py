...
class BaseModel(nn.Module):
    ...
    def pre_snapshot(self,optimizer, ss_id):
    ...
    def new_loss(self, head, rel, tail=None, label=None,first_layer_mask=None,second_layer_mask=None):
        """ return loss of new facts """
        return self.margin_loss(head, rel, tail, label,first_layer_mask,second_layer_mask) / head.size(0)

    def margin_loss(self, head, rel, tail, label=None,first_layer_mask=None,second_layer_mask=None):
        ...
        if self.args.using_multi_layer_distance_loss:
            if not 'first_layer_data_ent_embeddings_weight' in self._buffers:
                ent_embeddings = ent_embeddings*first_layer_mask.unsqueeze(1)
            if 'first_layer_data_ent_embeddings_weight' in self._buffers and not 'second_layer_data_ent_embeddings_weight' in self._buffers:
                ent_embeddings = ent_embeddings*second_layer_mask.unsqueeze(1)
