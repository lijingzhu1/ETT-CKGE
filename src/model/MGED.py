...
#     # Create a mask tensor initialized to 1
#     mask = torch.ones((n, dim))
#     # Calculate how many dimensions to mask (set to 0)
#     mask_dim = int(dim * float(mask_ratio))
#     for i in range(n):
#         # Randomly select mask_dim positions for each row and set them to 0
#         mask_indices = torch.randperm(dim)[:mask_dim]
...
class DLKGE(BaseModel):
    ...
    def pre_snapshot(self):
        ...
        if self.args.using_mask_weight and self.args.snapshot:
            ...
            self.mask_weight_linear = nn.Linear(self.args.emb_dim, self.args.emb_dim, bias=False).double()
            ...
            #     nn.Linear(self.args.emb_dim, self.args.emb_dim, bias=False).double()
            # )
            # for layer in self.mask_weight_linear:
            #     if isinstance(layer, nn.Linear):
            #         constant_(layer.weight, 1e-3)
            # # Move the entire module to CUDA
            # self.mask_weight_linear.cuda()
