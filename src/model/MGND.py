...
from torch_geometric.nn import GCN,GraphSAGE,GAT,PNA,EdgeCNN

def generate_node_mask(n, mask_ratio):
    # Create random ratios between 0 and 1 for each dimension
    random_ratios = torch.rand((n))
...
class DLKGE(BaseModel):
    ...
    def pre_snapshot(self):
        ...
        if self.args.using_mask_weight and self.args.snapshot:
            ...
            self.mask_weight_linear = GraphSAGE(in_channels=self.args.emb_dim, hidden_channels=200, \
...
class TransE(DLKGE):
    ...
    def MAE_loss(self, entity_mask, relation_mask, entity_mask_weight):
        ...
        new_node_mask = generate_node_mask(new_ent_embeddings.size(0),self.args.mask_ratio).to(self.args.device).double()
        new_ent_embeddings = new_node_mask.unsqueeze(1)*new_ent_embeddings
        # print(f'new_ent_embeddings:',new_ent_embeddings.size())
        # if edge_index.max() >= new_ent_embeddings.shape[0] or edge_index.min() < 0:
        #     print(f'edge_index.max:',edge_index.max())
        #     raise ValueError("edge_index contains out-of-bound indices.")
        new_ent_embeddings = self.mask_weight_linear(new_ent_embeddings.float(),edge_index = edge_index) 
        ...
        # print(f'new_ent_embedidngs,old_ent_embeddings:',new_ent_embeddings.size(),old_ent_embeddings.size())
        new_ent_embeddings = new_ent_embeddings*entity_mask.unsqueeze(1).double()
        old_ent_embeddings = old_ent_embeddings*entity_mask.unsqueeze(1)

        # loss = self.mse_loss_func(old_ent_embeddings, new_ent_embeddings)
        loss = self.huber_loss(old_ent_embeddings, new_ent_embeddings)/old_ent_len
        # if self.args.using_relation_distill:
        #     old_rel_embeddings = self.old_data_rel_embeddings_weight * relation_mask.unsqueeze(1)
        #     new_rel_embeddings = self.rel_embeddings.weight * relation_mask.unsqueeze(1)
        #     loss += self.huber_loss(old_rel_embeddings, new_rel_embeddings)
    ...
    def loss(self, head, relation, tail=None, label=None, entity_mask=None, relation_mask=None, entity_mask_weight=None,first_layer_mask=None,second_layer_mask=None):
        loss = 0.0;decompose_loss = 0.0;multi_layer_loss=0.0
