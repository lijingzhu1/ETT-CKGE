...
def generate_relation_mask(n, mask_ratio):
...
class DLKGE(BaseModel):
    ...
    def pre_snapshot(self):
        ...
        # self.mask_weight_linear = GCN(in_channels=self.args.emb_dim, hidden_channels=200, \
        #                 out_channels=self.args.emb_dim, num_layers=3)
        self.mask_weight_linear = nn.Sequential(
            nn.Linear(self.args.emb_dim, self.args.emb_dim/2, bias=False).double(),
            nn.ReLU(),  # Using Tanh activation
            nn.Linear(self.args.emb_dim/2, self.args.emb_dim, bias=False).double(),
        )
        constant_(self.mask_weight_linear[0].weight, 1e-3)
        constant_(self.mask_weight_linear[2].weight, 1e-3)
...
class TransE(DLKGE):
    ...
    def MAE_loss(self, entity_mask, relation_mask, entity_mask_weight):
        ...
        new_relation_mask = generate_relation_mask(new_rel_embeddings.size(0),self.args.mask_ratio).to(self.args.device).double()
        new_ent_embeddings = new_relation_mask.unsqueeze(1)*new_rel_embeddings
        ...
        new_rel_embeddings = self.mask_weight_linear(new_rel_embeddings) 
        ...
        # new_ent_embeddings = new_ent_embeddings
        # old_ent_embeddings = old_ent_embeddings*entity_mask.unsqueeze(1)
