...
class Tokened_BaseModel(nn.Module):
    def __init__(self, args, kg) -> None:
        super(Tokened_BaseModel, self).__init__()
        self.args = args
        self.kg = kg

        """ initialize the entity and relation embeddings for the first snapshot """
        self.ent_embeddings = nn.Embedding(self.kg.snapshots[0].num_ent, self.args.emb_dim).to(self.args.device)
        self.rel_embeddings = nn.Embedding(self.kg.snapshots[0].num_rel, self.args.emb_dim).to(self.args.device)
    ...
    def expand_embedding_size(self):
        ...
        ent_embeddings = nn.Embedding(self.kg.snapshots[self.args.snapshot + 1].num_ent, self.args.emb_dim).to(self.args.device)
        rel_embeddings = nn.Embedding(self.kg.snapshots[self.args.snapshot + 1].num_rel, self.args.emb_dim).to(self.args.device)
    ...
    def new_loss(self, head, rel, tail=None, label=None):
        ...
        return self.margin_loss(head, rel, tail, label,first_layer_mask,second_layer_mask) / head.size(0)
    ...
    def predict(self, head, relation, stage='Valid'):
        ...
        #     # print('hfeifhoah')
        #     ent_embeddings = self.old_data_ent_embeddings_weight_frezzed.clone().detach()
        #     rel_embeddings = self.old_data_rel_embeddings_weight_frezzed.clone().detach()
